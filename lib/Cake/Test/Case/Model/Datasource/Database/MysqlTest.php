 ...
 */
	public function testReadCustomJoinsAfterGeneratedJoins() {
		...
		$search = "LEFT JOIN $model8Table AS `TestModel8` ON " .
			...
			"LEFT JOIN $usersTable AS `Users` ON (`TestModel9`.`id` = `Users`.`test_id`)";
		...
		$test->read($this->Model, array(
			'joins' => array(
				array(
					...
					'alias' => 'Users',
					'type' => 'LEFT',
					'conditions' => array('TestModel9.id = Users.test_id')
				)
			),
			'recursive' => 1
		));
 ...
 */
	public function testArrayConditionsParsing() {
		...
		$result = $this->Dbo->conditions(array("Books.book_name" => 'Java(TM)'));
		$expected = " WHERE `Books`.`book_name` = 'Java(TM)'";
		$this->assertEquals($expected, $result);
		...
		$result = $this->Dbo->conditions(array("Books.id" => 0));
		$expected = " WHERE `Books`.`id` = 0";
		$this->assertEquals($expected, $result);

		$result = $this->Dbo->conditions(array("Books.id" => null));
		$expected = " WHERE `Books`.`id` IS NULL";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testArrayConditionsParsingComplexKeys() {
		$result = $this->Dbo->conditions(array(
			'CAST(Books.created AS DATE)' => '2008-08-02'
		));
		$expected = " WHERE CAST(`Books`.`created` AS DATE) = '2008-08-02'";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testVirtualFieldsWithSubquery() {
		$this->loadFixtures('Article', 'Comment', 'Users', 'Tag', 'ArticlesTag');
		$this->Dbo->virtualFieldSeparator = '__';
		$Article = ClassRegistry::init('Article');
		$commentsTable = $this->Dbo->fullTableName('comments', false, false);
