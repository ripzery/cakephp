 ...
 */
	public function testCountAfterFindCalls() {
		...
		$User = $this->getMock('Users', array('afterFind'), array(), '', true);
		...
		$User->expects($this->exactly(2)) // Article belongs to 1 Users and Primary belongs to 1 Users
			->method('afterFind')->with($this->anything(), $this->isFalse())->will($this->returnArgument(0));
 ...
 */
	public function testHasOneAfterFind() {
		...
		$User->Article = $Article;
		$User->find('first', array('conditions' => array('Users.id' => 1), 'recursive' => 2));
	}
}
